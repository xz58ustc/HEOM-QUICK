...
!                                                                       
      ...
      EXTERNAL DAXPBY, DDOT_LONG, DLAMCH, DNRM2_LONG, DQRDC, DQRSL, DRANDN, DROTG 
      EXTERNAL DSVDC, DUCPL1, DUCPL2, DUCPLL, DUCPLO, DUCPLU 
      DOUBLE PRECISION DDOT_LONG, DLAMCH, DNRM2_LONG, DUCPLL, DUCPLO, DUCPLU 
!                                                                       
...
!                                                                       
     ...
     &IDX(2,1)))                                                        
      ...
      R0 = DNRM2_LONG(NLEN,VECS(1,IDX(2,1)),1) 
...
!                                                                       
      DTMP1 = R0 
      DTMP2 = DNRM2_LONG(NLEN,VECS(1,IDX(3,1)),1) 
      IF (DTMP1.LT.TNRM) IERR = IERR + 16 
      IF (DTMP2.LT.TNRM) IERR = IERR + 32 
      IF (IERR.NE.0) GO TO 750 
      DWK(IDX(1,1),8*M+10)   = DONE 
      DWK(IDX(1,1),IDX(1,1)) = DDOT_LONG(NLEN,VECS(1,IDX(2,1)),1,VECS(1,IDX(3&
     &,1)),1) / ( DTMP1 * DTMP2 )                                       
...
!                                                                       
  190 INFO(2) = 1 
      ...
      RETLBL  = 200 
      RETURN 
  200 DWK(IDX(1,N),5*M+IDX(1,N)) = DDOT_LONG(NLEN,VECS(1,IDX(5,N)),1,VECS(1, &
...
!                                                                       
      DWK(IDX(1,N),8*M+16) = DNRM2_LONG(NLEN,VECS(1,IDX(4,N)),1) * DWK(IDX(1,&
     &N),8*M+14)                                                        
      DWK(IDX(1,N),8*M+17) = DNRM2_LONG(NLEN,VECS(1,IDX(5,N)),1) * DWK(IDX(1,&
     &N),8*M+15)                                                        
...
!                                                                       
     ...
     &)))                                                               
  260 END DO 
      DWK(IDX(1,NP1),8*M+16) = DNRM2_LONG(NLEN,VECS(1,3),1) * DWK(IDX(1,N),  &
     &8*M+14)                                                           
      DWK(IDX(1,NP1),8*M+17) = DNRM2_LONG(NLEN,VECS(1,4),1) * DWK(IDX(1,N),  &
     &8*M+15)                                                           
...
!                                                                       
  ...
  320    CONTINUE 
     ...
     &DTMP,VECS(1,IDX(5,I)))                                            
  330    CONTINUE 
         DWK(IDX(1,N),8*M+16) = DNRM2_LONG(NLEN,VECS(1,IDX(4,N)),1) *        &
     &DWK(IDX(1,N),8*M+14)                                              
         DWK(IDX(1,N),8*M+17) = DNRM2_LONG(NLEN,VECS(1,IDX(5,N)),1) *        &
     &DWK(IDX(1,N),8*M+15)                                              
      END IF 
!                                                                       
!     Step 11:                                                          
...
!                                                                       
  340 INFO(2) = 1 
      ...
      RETLBL  = 350 
      RETURN 
  350 DWK(IDX(1,N),5*M+IDX(1,N)) = DDOT_LONG(NLEN,VECS(1,IDX(5,N)),1,VECS(1, &
     &IDX(2,NP1)),1) * DWK(IDX(1,N),8*M+14) * DWK(IDX(1,N),8*M+15)      
!                                                                       
!     Step 10:                                                          
...
!                                                                       
  370 IF (N.LE.NUMCHK) THEN 
         DTMP1 = DNRM2_LONG(NLEN,VECS(1,IDX(2,NP1)),1) * DWK(IDX(1,N),8*M+14)&
     & / DWK(IDX(1,N),8*M+16)                                           
         DTMP2 = DNRM2_LONG(NLEN,VECS(1,IDX(3,NP1)),1) * DWK(IDX(1,N),8*M+15)&
     & / DWK(IDX(1,N),8*M+17)                                           
...
!                                                                       
  530 DTMP3      = DNRM2_LONG(NLEN,VECS(1,IDX(2,NP1)),1) 
      DTMP4      = DNRM2_LONG(NLEN,VECS(1,IDX(3,NP1)),1) 
      ...
      DWK(IDX(1,NP1),IDX(1,NP1)) = DDOT_LONG(NLEN,VECS(1,IDX(3,NP1)),1,VECS(1&
...
!                                                                       
     ...
     &)))                                                               
  590 END DO 
      IDTMP3       = DNRM2_LONG(NLEN,VECS(1,3),1) 
      IDTMP4       = DNRM2_LONG(NLEN,VECS(1,4),1) 
...
!                                                                       
  ...
  650    CONTINUE 
     ...
     &,-DTMP,VECS(1,IDX(3,I)))                                          
  660    CONTINUE 
         DTMP3      = DNRM2_LONG(NLEN,VECS(1,IDX(2,NP1)),1) 
         DTMP4      = DNRM2_LONG(NLEN,VECS(1,IDX(3,NP1)),1) 
         DTMP1      = DWK(IDX(1,N),8*M+14) * DTMP3 
         DTMP2      = DWK(IDX(1,N),8*M+15) * DTMP4 
         DWK(IDX(1,NP1),2*M+IDX(1,N)) = DTMP1 
...
!                                                                       
  ...
  720 CALL DAXPBY (NLEN,VECS(1,3),DONE,VECS(1,2),-DONE,VECS(1,3)) 
      RESN = DNRM2_LONG(NLEN,VECS(1,3),1) / R0 
      UCHK = RESN 
!                                                                       
