...
! Only lower half of zeroth-tier rho enters the linear problem to solve
!
do ni=1,nrho
   do nj=ni+1,nrho
      rho(ni,nj,1) = czero
...
!  
  if (mod(iter, iter_show) .eq. 0) then
     call calc_ax_tfqmr_comp(nrho, qmrvecs(1,1), qmrvecs(1,3))
     qmrvecs(1:lnum,3) = qmrvecs(1:lnum,3) - qmrvecs(1:lnum,2)
     derr0 = dnrm2(lnum, qmrvecs(1,3), 1)
     write(6,*)iter, derr0
...
!
  ...
  iter = iter + 1
  if (revcom .eq. 1) then
write(6,*)'solve_cplqmr: before ax'    
call flush(6)
     call calc_ax_tfqmr_comp(nrho, qmrvecs(1,colx), qmrvecs(1,colb))
write(6,*)'solve_cplqmr: after  ax'    
call flush(6)
  else 
write(6,*)'solve_cplqmr: before atrx'    
call flush(6)
     call calc_atrx_qmr(nrho, qmrvecs(1,colx), qmrvecs(1,colb))
write(6,*)'solve_cplqmr: after  atrx'    
call flush(6)
  end if
  call flush(6)
end do
...
!
call calc_ax_tfqmr_comp(nrho, rho(1,1,1), qmrvecs(1,3))
qmrvecs(1,3) = qmrvecs(1,3) - 1.d0
dtmp2 = dnrm2(lnum, qmrvecs(1,3), 1)
