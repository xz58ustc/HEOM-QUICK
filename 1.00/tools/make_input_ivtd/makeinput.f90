...
integer :: init, ntier, ncor, norbs, nspin, ialf, nrho
integer :: maxit0
integer :: fieldtype
integer :: icont, nresume
logical :: lresume, offcor, megaflux, readcp, readmat, lreadini
real*8  :: aoffL, aoffR, phioffL, phioffR
integer :: nalf
real*8  :: crit
real*8, allocatable  :: bandwidth(:), dinvbeta(:), linewidth(:,:), engyshift(:,:), tchar(:,:)
real*8, allocatable  :: dmtmp1(:,:), rhodiag(:)
real*8  :: tmax, dt
...
allocate(dmtmp1(norbs,norbs), rhodiag(nrho), STAT=istat)
!
tmax      = 1.d0
dt        = 0.02d0
!
...
if (lreadini) then
   write(7,*)
   write(7,461)lreadini
   write(7,333)(rhodiag(ni), ni=1,nrho)
   call flush(7)
end if
!
close(7)
...
461 format(' $readini  lreadini=', L7, 2x, '$end')
!
deallocate(bandwidth, dinvbeta, linewidth, engyshift, tchar, STAT=istat)
deallocate(dmtmp1, rhodiag, STAT=istat)
!
