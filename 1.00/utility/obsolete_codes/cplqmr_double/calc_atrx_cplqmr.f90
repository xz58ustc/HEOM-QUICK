subroutine calc_atrx_cplqmr(dim0, rcgtmp, rcgrhs)
!
...
!
! note :
!   Different from in <calc_atrx_bicg>, here the Hermicity of rho(*,*,1) is ignored 
!
use auxmod
use matmod
use tmpmatmod
...
if (dim0 .ne. nrho) then
  write(6,*)'calc_atrx_cplqmr: error! dim0 != nrho ', dim0, nrho
  stop
end if
dim1 = nrho**2
!
if (igroundsteady .eq. 0) then
  ...
  write(6,*)
  write(6,*)' error! unknown igroundsteady in calc_atrx_cplqmr ', igroundsteady
  stop
end if
!
...
!
    do ni=1,itier-1 
     ...
     if (igroundsteady .ne. 0) then
       ...
       !  cgamma_nk = cgamma_nk + eye * dipm(index_nk(ni)) * eleadinfty(ilead(index_nk(ni)), 1) 
       !else
...
!
end subroutine calc_atrx_cplqmr
