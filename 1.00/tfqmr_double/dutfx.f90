...
!                                                                       
!     Parameters:                                                       
!     For a description of  the parameters, see the file "dutfx.doc" in 
...
!********************************************************************** 
!                                                                       
      INTRINSIC DBLE, DSQRT, MAX0 
      EXTERNAL DLAMCH, DNRM2_LONG, DAXPBY, DDOT_LONG, DRANDN 
      DOUBLE PRECISION DDOT_LONG 
      DOUBLE PRECISION DLAMCH, DNRM2_LONG 
!                                                                       
!      INTEGER INFO(4), NDIM, NLEN, NLIM 
      INTEGER INFO(4), NLIM 
      integer*8  ndim, nlen
      DOUBLE PRECISION VECS(NDIM,9) 
...
!      PARAMETER (DHUN = 1.0D3,DONE = 1.0D0,DTEN = 1.0D1,DZERO = 0.0D0) 
!      PARAMETER (DHUN = 1.0D4,DONE = 1.0D0,DTEN = 1.0D1,DZERO = 0.0D0) 
!                                                                       
!     Local variables, permanent.                                       
!                                                                       
      INTEGER IERR, N, RETLBL, TF, TRES, VF 
      SAVE    IERR, N, RETLBL, TF, TRES, VF 
      DOUBLE PRECISION ALPHA, BETA, ETA, RHO 
...
!                                                                       
   20 DTMP = DDOT_LONG(NLEN,VECS(1,3),1,VECS(1,5),1) 
      BETA = DTMP / RHO 
      RHO  = DTMP 
!                                                                       
...
!                                                                       
      DTMP = DDOT_LONG(NLEN,VECS(1,3),1,VECS(1,4),1) 
...
!     \tau_{2n-1}^2.                                                    
!                                                                       
      DTMP = DNRM2_LONG(NLEN,VECS(1,5),1) 
      DTMP = DTMP * DTMP 
      VAR  = DTMP / TAU 
      COS  = DONE / ( DONE + VAR ) 
      TAU  = DTMP * COS 
...
!                                                                       
   ...
   40 CALL DAXPBY (NLEN,VECS(1,9),DONE,VECS(1,2),-DONE,VECS(1,9)) 
      RESN = DNRM2_LONG(NLEN,VECS(1,9),1) / R0 
      UCHK = RESN 
!                                                                       
