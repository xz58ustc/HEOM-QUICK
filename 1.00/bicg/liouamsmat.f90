subroutine liouamsmat(ialr, irhont, iorbs, ispin, isgn, alpha, zrho, lrho, &
                      const, zout, lout) 
use matmod
use tmpmatmod
implicit none
...
!
integer, intent(in)       :: ialr, irhont, iorbs, ispin, isgn, lrho, lout
...
if (ialr .eq. 1 .and. irhont .eq. 0) then  ! Y=A*rho
  do n1=1,nams
    ...
    do nj=1,nrho
!       zout(ni,nj) = zout(ni,nj) + dcmplx(0.d0, alpha*valams(n1,iorbs,ispin)) * zrho(nm,nj)
       zmtmp1(ni,nj) = zmtmp1(ni,nj) + dtmp0 * zrho(nm,nj)
    end do
  end do
!
  ...
  do n2=1,nams
    ...
    do ni=1,nrho
       zmtmp1(ni,nj) = zmtmp1(ni,nj) - dtmp0 * dconjg(zrho(nm,ni))
    end do
  end do
!
   ...
   do n2=1,nams
     ...
     dtmp0 = valams(n2,iorbs,ispin)
     do ni=1,nrho
        zmtmp1(ni,nj) = zmtmp1(ni,nj) + dtmp0 * zrho(ni,nn)
     end do
   end do
!
   ...
   do n1=1,nams
      ...
      dtmp0 = valams(n1,iorbs,ispin)
      do nj=1,nrho
         zmtmp1(ni,nj) = zmtmp1(ni,nj) - dtmp0 * dconjg(zrho(nj,nn))
      end do
   end do  
!
else
   write(6,*)'liouamsmat: error! unknown ialr = ', ialr, ' irhont = ', irhont
   stop
end if
!
zout(1:nrho, 1:nrho) = zout  (1:nrho, 1:nrho) * const +    &
                       zmtmp1(1:nrho, 1:nrho) * ctmp0
!
end subroutine liouamsmat
