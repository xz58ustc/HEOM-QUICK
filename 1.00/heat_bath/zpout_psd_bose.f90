subroutine zpout_psd_bose(mpole, zeig, zcoef, zpole)
!
! purpose : find poles and coefficients for PSD scheme for Bose function
! method  : [N-1/N] Pade decomposition of [J. Chem. Phys. 134, 244106 (2011)]
! 
! f(z) = 1/2 + 1/z + sum_i zcoef * 2 * z / (z^2 - zpole^2)
! with z = beta * w
!
! zpole is in the lower-half plane
!
implicit none
!
integer, intent (in)     :: mpole
complex*16, intent (out) :: zeig(*), zcoef(*), zpole(*)
...
do nj=1,dim1
   do ni=1,dim1
      if (ni .eq. nj + 1 .or. ni .eq. nj - 1) then
         dtmp0 = (2.d0 * dble(ni) + 3.d0) * (2.d0 * dble(nj) + 3.d0)
...
if (info .ne. 0) then
   write(6,*)'zpout_psd_bose: error! diagonalization failed for dmat0', info
   stop
end if
call dsyev('N', 'U', dim1, dmat1, dim1, dval1, dwork, lwork, info)
if (info .ne. 0) then
   write(6,*)'zpout_psd_bose: error! diagonalization failed for dmat1', info
   stop
end if
!
...
!write(6,*)'zpout_psd_bose: show dcoefeta'
do ni=1,n0
   dcoefeta(ni) = 5.d-1 * dble(n0 * (2 * n0 + 3)) * tmpa(ni)
!   write(6,100) ni, dcoefeta(ni)
   if (isnan(dcoefeta(ni))) then
      write(6,*)'zpout_psd_bose: error! NAN found for dcoefeta(', ni, ')'
      stop
   end if
end do
100 format('zpout_psd_bose: dcoefeta(', I4, ') = ', e14.6e3)
!
zcoef(1:n0) = dcoefeta(1:n0)
!
do ni=1,n0
